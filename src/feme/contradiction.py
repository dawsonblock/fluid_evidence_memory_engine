...
from .utils import new_id, now_iso, normalize_key

NEGATION_TERMS = {"not", "no", "never", "cannot", "can't", "isn't", "wasn't", "won't", "without"}
OPPOSITE_PREDICATES = {
    ("is", "is_not"),
    ("allowed", "not_allowed"),
...
class ContradictionEngine:
    ...
    def scan_new_claim(self, claim_id: str, *, con=None, autocommit: bool = True) -> list[dict]:
        con_ctx = nullcontext(con) if con is not None else self.db.connect()
        with con_ctx as active_con:
            claim = active_con.execute("SELECT * FROM memory_claims WHERE id = ?", (claim_id,)).fetchone()
            if not claim:
                return []
        ...
        for other in candidates:
            ...
            if detection:
                self.record(claim_id, other["id"], detection, con=con, autocommit=autocommit)
    ...
    def detect(self, a: dict, b: dict) -> dict | None:
        ...
        if same_pred and object_diff and (neg_a != neg_b or _conflicting_objects(a["object"], b["object"])):
            return {
    ...
    def record(self, claim_a_id: str, claim_b_id: str, detection: dict, *, con=None, autocommit: bool = True) -> None:
        now = now_iso()
        con_ctx = nullcontext(con) if con is not None else self.db.connect()
        with con_ctx as active_con:
            ...
            active_con.execute("UPDATE memory_claims SET status = 'disputed', updated_at = ? WHERE id IN (?, ?)", (now, claim_a_id, claim_b_id))
            if autocommit:
                active_con.commit()
...
def _conflicting_objects(a: str, b: str) -> bool:
    ...
    if aa == bb:
        return False
    if aa in {"true", "yes", "allowed", "enabled"} and bb in {"false", "no", "not_allowed", "disabled"}:
        return True
    if bb in {"true", "yes", "allowed", "enabled"} and aa in {"false", "no", "not_allowed", "disabled"}:
        return True
    return False
