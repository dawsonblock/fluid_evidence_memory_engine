...
class MaintenanceManager:
        ...
        with self.db.connect() as con:
            if str(getattr(self.db, "backend", "sqlite")).lower() == "postgres":
                ...
                return {
                    ...
                    "claims_indexed": int(claim_count),
            ...
            for row in rows:
                con.execute(
                    "DELETE FROM text_chunks_fts WHERE chunk_id = ?", (row["id"],)
                )
                con.execute(
                    "INSERT INTO text_chunks_fts (chunk_id, evidence_id, text) VALUES (?, ?, ?)",
                    (row["id"], row["evidence_id"], row["text"]),
                )
            con.commit()
        return {
            "project_id": project_id,
    ...
    def rebuild_embeddings(
        self, *, project_id: str = "default", owner_type: str = "chunk"
    ) -> dict:
        now = now_iso()
        count = 0
        with self.db.connect() as con:
            if owner_type == "chunk":
                ...
                for row in rows:
                    con.execute(
                        "DELETE FROM embeddings WHERE owner_type = 'chunk' AND owner_id = ?",
                        (row["id"],),
                    )
                    con.execute(
                        ...
                        (
                            ...
                            "chunk",
                            row["id"],
                            json.dumps(self.embedder.embed(row["text"])),
                ...
                rows = con.execute(
                    "SELECT id, claim_text FROM memory_claims WHERE project_id = ?",
                    (project_id,),
                ).fetchall()
                for row in rows:
                    con.execute(
                        "DELETE FROM embeddings WHERE owner_type = 'claim' AND owner_id = ?",
                        (row["id"],),
                    )
                    con.execute(
                        "INSERT INTO embeddings (id, owner_type, owner_id, vector_json, model, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                        (
                            ...
                            "claim",
                            row["id"],
                            json.dumps(self.embedder.embed(row["claim_text"])),
                            "hashing-embedding-v1",
                            now,
                        ),
                    )
