...
class CitationManager:
    ...
    def citations_for_context(
        self, packet: ContextPacket, *, persist: bool = False
    ) -> list[dict]:
        citations: list[dict] = []
        ...
        for item in packet.included:
            if item.get("kind") == "claim":
                for ev in item.get("supporting_evidence") or []:
                    ...
                    seen.add(key)
                    citations.append(
                        self._citation_from_link(
                            ev, claim_id=item.get("claim_id"), text=item.get("text")
                        )
                    )
        ...
        citations = [
            {**c, "citation_label": f"C{i + 1}"} for i, c in enumerate(citations)
        ]
        if persist:
            self.persist_citations(
                packet.question,
                citations,
                project_id=str(packet.metadata.get("project_id", "default")),
            )
        return citations

    def persist_citations(
        self, question: str, citations: list[dict], *, project_id: str = "default"
    ) -> dict:
        now = now_iso()
    ...
    def list_records(
        self, *, citation_set_id: str | None = None, limit: int = 100
    ) -> list[dict]:
        with self.db.connect() as con:
            if citation_set_id:
                rows = con.execute(
                    "SELECT * FROM citation_records WHERE citation_set_id = ? ORDER BY citation_label LIMIT ?",
                    (citation_set_id, limit),
                ).fetchall()
            else:
                rows = con.execute(
                    "SELECT * FROM citation_records ORDER BY created_at DESC LIMIT ?",
                    (limit,),
                ).fetchall()
        return rows_to_dicts(rows)

    def _citation_from_link(
        self, link: dict, *, claim_id: str | None, text: str | None
    ) -> dict:
    ...
    def _spans(self, span_ids: list[str]) -> list[dict]:
        ...
        with self.db.connect() as con:
            rows = con.execute(
                f"SELECT * FROM token_spans WHERE id IN ({ph})", span_ids
            ).fetchall()
        return rows_to_dicts(rows)
