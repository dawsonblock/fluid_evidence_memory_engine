...
def make_database(db: str | None = None, *, backend: str | None = None) -> Any:
    ...
    if selected_backend == "postgres" or str(target).startswith(("postgres://", "postgresql://")):
        dsn = db or settings.postgres_dsn or settings.db_path
        if not dsn or not str(dsn).startswith(("postgres://", "postgresql://")):
            raise ValueError("PostgreSQL backend selected but no DSN was provided. Set FEME_POSTGRES_DSN or pass --db postgresql://...")
...
def runtime_health(database: Any | None = None) -> dict:
    ...
    return {"health": health.__dict__, "capabilities": caps.__dict__}
